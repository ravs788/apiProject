...
public class ElasticLowLevelRestClientFactory {
    ...
    public static RestClient getRestClient(ElasticServerChoices ELASTIC_SERVER){
        ...
        switch (ELASTIC_SERVER){
            case ON_CLOUD: return getRestClientForCloud();
            case ON_LOCALHOST_SECURE: return getRestClientForLocalhostHTTPS();
            case ON_LOCALHOST_INSECURE: return getRestClientLocalhostHTTP();
            default: throw new IllegalStateException(String.format("%s is not a valid HOST choice. Pick your HOST from %s.",ELASTIC_SERVER, java.util.Arrays.asList(ElasticServerChoices.class)));
        }
    }
